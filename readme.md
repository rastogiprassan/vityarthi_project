...
OVERVIEW OF THE PROJECT : This Python taxi fare estimator project is designed to compute the total taxi fare for a trip by incorporating key variables such as trip distance, passenger count, vehicle type, round trip indicator, and waiting time. It uses predefined base fares and per kilometer rates for multiple vehicle categoris (HATCHBACK, SEDAN, SUV, PREMIUM SEDAN), allowing the fare calculation to reflect different pricing tiers.
...
3.  Dynamic rate look-up for diferent vehicle types using a dictionary.
